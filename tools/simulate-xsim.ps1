...
$sources = @(
    (Join-Path $projectRoot 'src\adder_4bit.v'),
    (Join-Path $projectRoot 'sim\tb_adder_4bit.sv')
)
...
try {
    ...
    & $elaborator tb_adder_4bit --debug typical --snapshot adder_4bit_sim *> elaborate.log
    if ($LASTEXITCODE -ne 0) { Get-Content elaborate.log; throw 'Elaboration failed.' }
    & $simulator adder_4bit_sim --runall *> simulation.log
    $simExit = $LASTEXITCODE
    Get-Content simulation.log
    if ($simExit -ne 0) { throw 'Simulation failed.' }
    if (-not (Select-String -LiteralPath simulation.log -SimpleMatch 'LAB1_PASS adder_4bit cases=256' -Quiet)) { throw 'The testbench did not report all 256 cases passing.' }
    if (-not (Test-Path -LiteralPath 'wave.vcd')) { throw 'No VCD was generated.' }
    Copy-Item -LiteralPath 'wave.vcd' -Destination $waveOutput
    Write-Host 'SIMULATED: LAB1_PASS adder_4bit cases=256'
